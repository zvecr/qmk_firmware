...
#    define eeprom_printf(fmt, ...) if(debug_enable) xprintf(fmt, ##__VA_ARGS__);
#else /* NO_DEBUG */
...
static uint32_t get_virtual_eeprom_size(void) {
    ...
    if (virtual_eeprom_size == UINT32_MAX) {
        virtual_eeprom_size = VIRTUAL_EEPROM_MAP[NVMCTRL->SEESTAT.bit.SBLK][NVMCTRL->SEESTAT.bit.PSZ];
    }
