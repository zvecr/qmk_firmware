...
static const connection_host_t available_hosts[] = {
...
#define AVAILABLE_HOSTS_COUNT ARRAY_SIZE(available_hosts)
...
void connection_init(void) {
    ...
    if (desired_host == CONNECTION_HOST_INVALID) {
...
void connection_next_host_noeeprom(void) {
    uint8_t next = 0;
    for (uint8_t i = 0; i < AVAILABLE_HOSTS_COUNT; i++) {
        if (available_hosts[i] == config.desired_host) {
            next = i == AVAILABLE_HOSTS_COUNT - 1 ? 0 : i + 1;
            break;
        }
    }

    connection_set_host_noeeprom(available_hosts[next]);
}
...
void connection_prev_host_noeeprom(void) {
    ...
    for (uint8_t i = 0; i < AVAILABLE_HOSTS_COUNT; i++) {
        if (available_hosts[i] == config.desired_host) {
            next = i == 0 ? AVAILABLE_HOSTS_COUNT - 1 : i - 1;
            break;
        }
    }
...
connection_host_t connection_get_host(void) {
    if (desired_host == CONNECTION_HOST_AUTO) {
